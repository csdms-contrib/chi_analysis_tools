# -*- coding: utf-8 -*-
"""
Created on Sun Aug 25 07:19:29 2013
...
def make_plots():
    ...
    for fname in glob(DataDirectory+"*_fullProfileMC_colinear_*.tree"): # assigns a number to each iteration (i.e. for every .tree file in the directory)
    #for fname in glob(DataDirectory+"*_mover_n_*"): # assigns a number to each iteration (i.e. for every .tree file in the directory)
        ...
        plt.savefig(OutputFigureName+'_Mchi.' + OutputFigureFormat, format=OutputFigureFormat)
        plt.clf()       
